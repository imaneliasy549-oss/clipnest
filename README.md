...
> the window you came from. Install the package with
> `sudo apt install ./clipnest_0.7.0-1_amd64.deb`, run `clipnest setup`, log out
> and back in once. `clipnest help` lists every command, `clipnest config` shows
> the settings, `clipnest doctor` explains what is wrong. See `CHANGELOG.md` for
> what each version changed, and `RELEASING.md` for building and publishing.
...
```bash
sudo apt install ./clipnest_0.7.0-1_amd64.deb   # یا: sudo dpkg -i …
...
| توزیع | وضعیت |
| --- | --- |
| Ubuntu 24.04 (GNOME 46) و جدیدتر | ✅ با همین بسته |
...
| معماری arm64 | از ورک‌فلوی گیت‌هاب (`release.yml`) بستهٔ `arm64` هم ساخته می‌شود |
...
```bash
make check        # clippy با -D warnings + تست‌ها
...
نیمی از تست‌های مربوط به Portal دیالوگ باز می‌کنند، پس در حالت عادی اجرا نمی‌شوند؛
برای بررسی دست‌ی‌دادن با Portal واقعی (بدون باز کردن دیالوگ):
...
نسخه‌های قدیمی، پین، هرس، آزادسازی فضا، رفت‌وبرگشت هشتصویر و پشتیبان‌گیری، پارسر تنظیمات،
سیاست eviction کش بندانگشتی، اعتبارسنجی میانبر و هم‌خوانی فایل‌های بسته با کد را
پوشش می‌دهند.
